...
app.put('/movies/:id', async (req, res) => {
   ...
   try {
      ...
      return res.status(500).send({ message: 'Falha ao atulizar o resgitro do filme!' });
   }
...
app.delete('/movies/:id', async (req, res) => {
   ...
   try {
      ...
      return res.status(500).send({ message: 'Falhar ao deletar o filme' });
   }
   res.status(200).send();
});
...
app.get('/movies/:genderName', async (req, res) => {
   try {
      ...
      return res.status(500).send({ message: 'Ops!!, filme não filtrado pelo gênero!' });
   }
});
